...
// Global store for manual table overrides (loaded from Firebase)
...
function startRealtimeListeners() {
    ...
    // Table Overrides (manual edits from edit-table.html)
    window.db.collection("tableOverrides").onSnapshot(snap => {
        ...
        // Re-render table with updated overrides
        window.db.collection("matches").get().then(mSnap => {
            const matches = [];
            mSnap.forEach(doc => matches.push(docToMatch(doc)));
            matches.sort((a,b) => (a.gameNumber||0)-(b.gameNumber||0));
            renderLeagueTable(matches);
        });
...
function waitForDB(callback) {
    ...
    const interval = setInterval(() => {
        ...
        if (window.db) {
            ...
            clearInterval(interval);
            console.error("❌ Firebase DB not available after 5s. Check firebase.js");
            // Show error on page if visible
