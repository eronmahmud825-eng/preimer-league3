...
function applyMatchToTeams(match, teams) {
    ...
    if (match.score1 > match.score2) { a.win++; b.lose++; }
    else if (match.score1 < match.score2) { b.win++; a.lose++; }
    else { a.draw++; b.draw++; }
}
...
function getForm(teamName, matches) {
    return matches.filter(m => m.team1 === teamName || m.team2 === teamName)
        .sort((a, b) => (b.gameNumber||0) - (a.gameNumber||0)).slice(0,5).reverse()
        .map(m => {
...
function renderLeagueTable(matches) {
    ...
    computeTeamsFromMatches(matches).forEach((t, idx) => {
        ...
        const formHtml = form.length ? form.map(r => formDot(r)).join('') : '<span style="color:#2d4a65;font-size:0.7rem;">-</span>';
...
function renderInjuryTable(injuries) {
    ...
    injuries.filter(i => i.injuryGamesLeft > 0).forEach(inj => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="Actions" style="white-space:nowrap;">
                <button class="edit-btn" data-id="${inj.id}" data-player="${inj.player}" data-games="${inj.injuryGamesLeft}">✏️ Edit</button>
                <button class="delete-btn" data-id="${inj.id}" data-player="${inj.player}">🗑 Delete</button>
            </td>`;
        tbody.appendChild(tr);
    });
    injuries.filter(i => i.injuryGamesLeft === 0).forEach(inj => {
...
function openEditInjuryModal(id, player, gamesLeft) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `<div style="background:#1e2a3a;padding:24px;border-radius:14px;min-width:300px;color:#fff;box-shadow:0 8px 40px #000c;">
            <h3 style="margin-top:0;">✏️ Edit Injury</h3>
            <p id="editInjuryLabel" style="color:#aaa;margin:0 0 14px;font-size:0.9em;"></p>
            <label style="display:block;margin:10px 0;font-size:0.9em;">🏥 Games Left (0 = recovered):
                <input id="editInjuryGames" type="number" min="0" max="20" style="margin-left:8px;width:60px;padding:5px;border-radius:6px;border:none;background:#2d3f55;color:#fff;"/>
            </label>
            <div style="display:flex;gap:10px;margin-top:12px;">
                <button id="saveEditInjuryBtn" style="background:#e67e22;color:#fff;border:none;padding:9px 18px;border-radius:8px;cursor:pointer;">💾 Save</button>
                <button id="cancelEditInjuryBtn" style="background:#555;color:#fff;border:none;padding:9px 18px;border-radius:8px;cursor:pointer;">Cancel</button>
            </div></div>`;
    ...
    document.getElementById("saveEditInjuryBtn").onclick = () => {
        ...
        if (!window.db) return;
        window.db.collection("playerInjuries").doc(modal._editId).update({ injuryGamesLeft: games })
            .then(() => { modal.style.display = "none"; alert("✅ Injury updated!"); })
...
function deleteInjury(id, player) {
    ...
    window.db.collection("playerInjuries").doc(id).delete().then(() => alert("✅ Deleted")).catch(() => alert("❌ Error"));
}
...
function openEditSuspensionModal(susp) {
    ...
    if (!modal) {
        ...
        modal.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.75);display:flex;align-items:center;justify-content:center;z-index:9999;";
        modal.innerHTML = `<div style="background:#1e2a3a;padding:24px;border-radius:14px;min-width:320px;color:#fff;box-shadow:0 8px 40px #000c;">
            ...
            <label style="display:block;margin:10px 0;font-size:0.9em;">🟡 Yellow Ban Left:
            ...
            <label style="display:block;margin:10px 0;font-size:0.9em;">🔴 Red Ban Left:
    ...
    document.getElementById("saveEditSuspBtn").onclick = () => {
        ...
        }).then(() => { modal.style.display = "none"; alert("✅ Updated!"); }).catch(() => alert("❌ Error"));
    };
}
...
function onDeleteMatchClick(docId) {
    const pass = prompt("Admin password:");
    if (pass !== ADMIN_PASSWORD && pass !== SUPER_ADMIN_PASSWORD) { alert("❌ Wrong password"); return; }
    window.db.collection("matches").doc(docId).delete().then(() => alert("✅ Match deleted")).catch(() => alert("❌ Error"));
}
...
function saveCardWarning(matchTeam1, matchTeam2) {
    ...
    if (!team || !player || !cardTypeRadio) { alert("❌ Fill all card fields"); return; }
    ...
    getOrCreatePlayerSusp(team, player).then(susp => {
        ...
        alert(cardType === "red" ? `✅ 🔴 Red card — ${player} banned 3 matches` : `✅ 🟡 Yellow card added for ${player}`);
...
function saveInjury(matchTeam1, matchTeam2) {
    ...
    getOrCreatePlayerInjury(team, player).then(inj => {
        ...
        const label = injuryTypeRadio.value === "easy" ? "🟡 Easy — out 3 matches" : "🔴 Hard — out 5 matches";
        alert(`✅ 🏥 ${player} — ${label}`);
...
function checkGameEligibility() {
    ...
    Promise.all([
        ...
        if (suspended.length) {
            html += `<h4 style="color:#e74c3c;margin:14px 0 8px;">⛔ Suspended:</h4>`;
            suspended.forEach(({s, type}) => {
                if (type === "red")
                    html += `<div style="background:#1e0a0a;border-left:4px solid #e74c3c;padding:12px;margin:6px 0;border-radius:8px;"><strong>${s.player}</strong> <span style="color:#999;font-size:0.85em;">(${s.team})</span><br/><span style="font-size:0.92em;">🔴 Red ban — <strong>${s.redBanLeft} match(es) left</strong></span></div>`;
                else
                    html += `<div style="background:#1a1400;border-left:4px solid #f1c40f;padding:12px;margin:6px 0;border-radius:8px;"><strong>${s.player}</strong> <span style="color:#999;font-size:0.85em;">(${s.team})</span><br/><span style="font-size:0.92em;">🟡 Yellow ban — <strong>${s.yellowBanLeft} match left</strong></span></div>`;
            });
        }
        if (injured.length) {
            ...
            injured.forEach(inj => {
                const isHard = inj.injuryGamesLeft > 3;
                html += `<div style="background:${isHard?'#1e0a0a':'#1e1200'};border-left:4px solid ${isHard?'#e74c3c':'#f39c12'};padding:12px;margin:6px 0;border-radius:8px;"><strong>${inj.player}</strong> <span style="color:#999;font-size:0.85em;">(${inj.team})</span><br/><span style="font-size:0.92em;">${isHard?'🔴 Hard':'🟡 Easy'} Injury — <strong>${inj.injuryGamesLeft} match(es) left</strong></span></div>`;
            });
        }
        if (warnings.length) {
            html += `<h4 style="color:#f39c12;margin:14px 0 8px;">⚠️ Yellow Warnings:</h4>`;
            warnings.forEach(s => {
...
function setupMatchWeek() {
    ...
    if (saveBtn) {
        saveBtn.addEventListener("click", () => {
            ...
            window.db.collection("matches").get().then(snap => {
                ...
                return window.db.collection("matches").add({ team1, team2, score1, score2, date, gameNumber, savedAt: new Date().toLocaleString() })
    ...
    if (okBtn)         okBtn.addEventListener("click", () => { alert("✅ All done!"); window.location.href = "index.html"; });
}
...
function generateMatchReport(team1, team2, score1, score2, gameNumber) {
    ...
    const headlines = winner
        ? [`${winner} یارییەکەی بەسەر ${loser}دا دەبات بە ئەنجامی ${s1}-${s2}`, `GW${gameNumber}: ${winner} سێ خاڵ وەردەگرێت`]
        : [`ماچی هەیجانئەنگیز: ${team1} و ${team2} خاڵ دابەشدەکەن`, `GW${gameNumber}: مساواتی ${s1}-${s2}`];
    const headline = headlines[Math.floor(Math.random() * headlines.length)];
    const body = winner
        ? `لە جۆلەی یارییە ${gameNumber}مدا، ${winner} سەرکەوتنێکی بەرچاوی بەدەستهێنا لە دژی ${loser}دا بە ئەنجامی ${s1}-${s2}دا.`
        : `لە جۆلەی یارییە ${gameNumber}مدا، ${team1} و ${team2} بە مساواتی ${s1}-${s2} جیابوونەوە.`;
    window.db.collection("matchReports").add({ team1, team2, score1: s1, score2: s2, gameNumber, report: `📰 ${headline}\n\n${body}`, createdAt: new Date().toISOString() }).catch(console.error);
}

function renderMatchReports(reports) {
    const container = document.getElementById("reportsContainer");
    if (!container) return;
    if (!reports.length) { container.innerHTML = `<div style="text-align:center;padding:40px;color:#2d4a65;">هیچ ڕاپۆرتێک نییە</div>`; return; }
    container.innerHTML = "";
    reports.forEach(r => {
        ...
        card.innerHTML = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;"><span style="font-size:0.85rem;background:rgba(0,200,83,0.1);border:1px solid rgba(0,200,83,0.2);border-radius:16px;padding:3px 10px;color:#00c853;">GW${r.gameNumber}</span><span style="font-size:0.9rem;font-weight:700;color:#fff;">${r.team1} <span style="color:#00c853;">${r.score1} - ${r.score2}</span> ${r.team2}</span><span style="margin-left:auto;font-size:0.7rem;color:#2d4a65;">${date}</span></div><div style="font-size:0.92rem;line-height:1.7;color:#c8d8e8;direction:rtl;text-align:right;white-space:pre-wrap;">${r.report}</div>`;
...
function setupAdminPanel() {
    ...
    if (addPlayerBtn) {
        addPlayerBtn.addEventListener("click", () => {
            ...
            if (!team || !player) { alert("❌ Fill all fields"); return; }
...
window.deletePlayer = function(id) {
    if (prompt("Admin password:") !== ADMIN_PASSWORD && prompt("Admin password:") !== SUPER_ADMIN_PASSWORD) return;
    window.db.collection("players").doc(id).delete().then(() => { alert("✅ Deleted"); loadPlayersTable(); });
};
...
function loadEncounters(matches) {
    ...
    const results = {};
    matches.forEach(m => {
        [m.team1, m.team2].forEach(t => { if (!results[t]) results[t] = {}; });
        if (!results[m.team1][m.team2]) results[m.team1][m.team2] = {wins:0,losses:0};
        if (!results[m.team2][m.team1]) results[m.team2][m.team1] = {wins:0,losses:0};
...
function startRealtimeListeners() {
    ...
    // Pre-load overrides then start match listener
...
function _attachMatchListener() {
    ...
    window.db.collection("matches").orderBy("gameNumber","asc").onSnapshot(snap => {
        ...
        console.warn("Fallback unordered:", err.message);
...
function waitForDB(callback) {
    ...
    const iv = setInterval(() => {
        attempts++;
        if (window.db) { clearInterval(iv); callback(window.db); }
        else if (attempts > 50) {
            clearInterval(iv);
