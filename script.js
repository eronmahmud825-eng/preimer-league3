...
function openEditInjuryModal(id, player, gamesLeft) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = '<div style="background:#1e2a3a;padding:24px;border-radius:14px;min-width:300px;color:#fff;box-shadow:0 8px 40px #000c;"><h3 style="margin-top:0;">Edit Injury</h3><p id="editInjuryLabel" style="color:#aaa;margin:0 0 14px;font-size:0.9em;"></p><label style="display:block;margin:10px 0;font-size:0.9em;">Games Left (0 = recovered): <input id="editInjuryGames" type="number" min="0" max="20" style="margin-left:8px;width:60px;padding:5px;border-radius:6px;border:none;background:#2d3f55;color:#fff;"/></label><div style="display:flex;gap:10px;margin-top:12px;"><button id="saveEditInjuryBtn" style="background:#e67e22;color:#fff;border:none;padding:9px 18px;border-radius:8px;cursor:pointer;">Save</button><button id="cancelEditInjuryBtn" style="background:#555;color:#fff;border:none;padding:9px 18px;border-radius:8px;cursor:pointer;">Cancel</button></div></div>';
...
function saveInjury(matchTeam1, matchTeam2) {
    ...
    getOrCreatePlayerInjury(team, player).then(inj => {
        ...
        alert(player + (injuryType === "easy" ? " — Easy Injury, out for 1 match" : " — Hard Injury, out for 5 matches"));
    ...
    Promise.all([
        ...
        if (suspended.length > 0) {
            ...
            suspended.forEach(function(item) {
                ...
                if (type === "red") html += '<div style="background:#1e0a0a;border-left:4px solid #e74c3c;padding:12px;margin:6px 0;border-radius:8px;"><strong>' + s.player + '</strong> <span style="color:#999;font-size:0.85em;">(' + s.team + ')</span><br/><span style="font-size:0.92em;">Red card ban — <strong>' + s.redBanLeft + ' match(es) left</strong></span></div>';
...
function generateMatchReport(team1, team2, score1, score2, gameNumber) {
    ...
    if (winner) {
        ...
        body += "یارییەکە بە " + goalWord + " تەواو بوو. ";
        if (s1 > s2 + 1 || s2 > s1 + 1) body += winner + " بە شێوەیەکی بەهێز سەرکەوتوو بوو. ";
        else body += loser + " تا دواترین خولەکەکان بەرجەستە ماوە. ";
        body += "\n\nئەم سەرکەوتنە " + winner + " هێزێکی زیاتر دەبەخشێت بۆ قۆناغە داهاتووەکان.";
    } else {
        body = "لە جۆلەی یارییە " + gameNumber + "مدا، " + team1 + " و " + team2 + " بە مساواتی " + s1 + "-" + s2 + " جیابوونەوە.\n\n";
    ...
    }).then(() => console.log("Match report saved"))
    ...
    reports.forEach((r) => {
        ...
        card.innerHTML =
            '<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap;">' +
                '<span style="font-size:0.85rem;background:rgba(0,200,83,0.1);border:1px solid rgba(0,200,83,0.2);border-radius:16px;padding:3px 10px;color:#00c853;">GW' + r.gameNumber + '</span>' +
                '<span style="font-size:0.9rem;font-weight:700;color:#fff;">' + r.team1 + ' <span style="color:#00c853;">' + r.score1 + ' - ' + r.score2 + '</span> ' + r.team2 + '</span>' +
                winBadge +
                '<span style="margin-left:auto;font-size:0.7rem;color:#2d4a65;">' + date + '</span>' +
            '</div>' +
        ...
        window.db.collection("matches").get().then(snap => {
            ...
            docs.forEach(doc => {
                ...
                tr.innerHTML =
                    ...
                    '<td style="white-space:nowrap;"><input type="number" min="0" id="s1_' + doc.id + '" value="' + (d.score1||0) + '" style="width:46px;padding:4px;border-radius:6px;border:none;background:#1a2d42;color:#fff;text-align:center;"/> - <input type="number" min="0" id="s2_' + doc.id + '" value="' + (d.score2||0) + '" style="width:46px;padding:4px;border-radius:6px;border:none;background:#1a2d42;color:#fff;text-align:center;"/></td>' +
                    ...
                    '<td style="white-space:nowrap;"><button onclick="saUpdateMatch(\'' + doc.id + '\')" style="background:#27ae60;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;margin-right:4px;">Save</button><button onclick="saDeleteMatch(\'' + doc.id + '\')" style="background:#c0392b;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;">Delete</button></td>';
        ...
        window.db.collection("playerSuspensions").get().then(snap => {
            ...
            snap.forEach(doc => {
                ...
                tr.innerHTML =
                    ...
                    '<td style="white-space:nowrap;"><button onclick="saUpdateSusp(\'' + doc.id + '\')" style="background:#27ae60;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;margin-right:4px;">Save</button><button onclick="saDeleteSusp(\'' + doc.id + '\',\'' + (d.player||'').replace(/'/g,"") + '\')" style="background:#c0392b;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;">Delete</button></td>';
        ...
        window.db.collection("playerInjuries").get().then(snap => {
            ...
            snap.forEach(doc => {
                ...
                tr.innerHTML =
                    ...
                    '<td style="white-space:nowrap;"><button onclick="saUpdateInj(\'' + doc.id + '\')" style="background:#27ae60;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;margin-right:4px;">Save</button><button onclick="saDeleteInj(\'' + doc.id + '\',\'' + (d.player||'').replace(/'/g,"") + '\')" style="background:#c0392b;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;">Delete</button></td>';
        ...
        window.db.collection("players").get().then(snap => {
            ...
            snap.forEach(doc => {
                ...
                tr.innerHTML =
                    ...
                    '<td style="white-space:nowrap;"><button onclick="saUpdatePlayer(\'' + doc.id + '\',\'' + (d.team||'').replace(/'/g,"") + '\')" style="background:#27ae60;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;margin-right:4px;">Save</button><button onclick="saDeletePlayer(\'' + doc.id + '\')" style="background:#c0392b;color:#fff;border:none;padding:5px 12px;border-radius:6px;cursor:pointer;">Delete</button></td>';
